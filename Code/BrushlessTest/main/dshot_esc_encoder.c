...
/**
 * @brief Type of DShot ESC frame (16-bit)
 */
typedef union {
    struct {
...
static void make_dshot_frame(dshot_esc_frame_t *frame, uint16_t throttle, bool telemetry)
{
    ...
    frame->crc = crc;
    val = frame->val;
    /* change the endian so MSB of frame is sent first by bytes encoder */
...
esp_err_t rmt_new_dshot_esc_encoder(const dshot_esc_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder)
{
    ...
    ESP_GOTO_ON_FALSE(dshot_encoder, ESP_ERR_NO_MEM, err, TAG, "no mem for dshot encoder");
    ...
    /* DShot bit timing: bit1 ~74.85% high, bit0 ~37.425% high */
    ...
    ESP_GOTO_ON_ERROR(rmt_new_bytes_encoder(&bytes_encoder_config, &dshot_encoder->bytes_encoder),
                      err, TAG, "create bytes encoder failed");
    rmt_copy_encoder_config_t copy_encoder_config = {};
    ESP_GOTO_ON_ERROR(rmt_new_copy_encoder(&copy_encoder_config, &dshot_encoder->copy_encoder),
